...
export class ProductsService {
  ...
  async addVendors(productId: string, vendorIds: string[]): Promise<Product> {
    const product = await this.productModel
      .findByIdAndUpdate(
        productId,
  ...
  async removeVendors(productId: string, vendorIds: string[]): Promise<Product> {
    const product = await this.productModel
      .findByIdAndUpdate(
        productId,
  ...
  async getVendors(productId: string) {
    const product = await this.productModel
      .findById(productId)
      .populate('vendors', 'name ratings')
      .exec();
