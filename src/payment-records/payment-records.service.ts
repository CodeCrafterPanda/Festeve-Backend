...
export class PaymentRecordsService {
  ...
  async create(userId: string, createPaymentRecordDto: CreatePaymentRecordDto): Promise<PaymentRecord> {
    ...
    // Add payment record to the related order or booking
    if (createPaymentRecordDto.relatedTo === 'order') {
      await this.ordersService.addPaymentRecord(
        createPaymentRecordDto.referenceId, 
        (savedPaymentRecord._id as string).toString()
      );
    } else if (createPaymentRecordDto.relatedTo === 'booking') {
      await this.bookingsService.addPaymentRecord(
        createPaymentRecordDto.referenceId, 
        (savedPaymentRecord._id as string).toString()
      );
    }
  ...
  async updateStatus(id: string, status: string, paidAt?: Date): Promise<PaymentRecord> {
    ...
    // Recalculate payment status for the related order or booking
    if (paymentRecord.relatedTo === 'order') {
      await this.ordersService.calculatePaymentStatus(paymentRecord.referenceId.toString());
    } else if (paymentRecord.relatedTo === 'booking') {
      await this.bookingsService.calculatePaymentStatus(paymentRecord.referenceId.toString());
    }
